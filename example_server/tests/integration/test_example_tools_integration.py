    ...
    async def test_tool_exception_handling(self, mcp_session):
        ...
        try:
            ...
            if isinstance(data, dict) and data.get("Status") == "Exception":
                ...
                assert "Message" in data, f"SAAGA error missing Message (transport: {transport})"
                assert "ExceptionType" in data, f"SAAGA error missing ExceptionType (transport: {transport})"
                assert "Traceback" in data, f"SAAGA error missing Traceback (transport: {transport})"
        except json.JSONDecodeError:
