...
from fixed_mcp_server.logging_config import setup_logging, logger
    ...
    for tool_func in example_tools:
        ...
        if tool_func.__name__ == "echo_tool":
            ...
            def echo_tool_wrapper(message: str) -> types.TextContent:
                ...
                logger.debug(f"echo_tool_wrapper called with message: {message!r}")
                logger.debug(f"echo_tool_wrapper args type: {type(message)}")
                try:
                    result = decorated_func(message)
                    logger.debug(f"echo_tool decorated_func returned: {result!r}")
                    ...
                    logger.error(f"echo_tool_wrapper error: {e}", exc_info=True)
                    raise
