"""Test Config Server - MCP Server with SAAGA Decorators
...
from test_config_server.config import ServerConfig, get_config
from test_config_server.logging_config import setup_logging, logger
from test_config_server.tools.example_tools import example_tools, parallel_example_tools
def create_mcp_server(config: Optional[ServerConfig] = None) -> FastMCP:
    ...
    mcp_server = FastMCP(config.name or "Test Config Server")
...
def register_tools(mcp_server: FastMCP, config: ServerConfig) -> None:
    ...
    from test_config_server.decorators.exception_handler import exception_handler
    from test_config_server.decorators.tool_logger import tool_logger
    from test_config_server.decorators.parallelize import parallelize
...
def main(port: int, transport: str) -> int:
    """Run the Test Config Server server with specified transport."""
    try:
        if transport == "stdio":
