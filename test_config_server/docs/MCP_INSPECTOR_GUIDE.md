# MCP Inspector Testing Guide for Test Config Server
...
which mcp
# Should show: /path/to/test_config_server/.venv/bin/mcp
...
1. Use the full path to your venv's mcp:
   ```bash
   .venv/bin/mcp dev test_config_server/server/app.py
   ```
...
```bash
mcp dev test_config_server/server/app.py
```
...
**Text Logs**:
- macOS: `~/Library/Logs/mcp-servers/test_config_server.log`
- Linux: `~/.local/state/mcp-servers/logs/test_config_server.log`
- Windows: `%LOCALAPPDATA%\mcp-servers\logs\test_config_server.log`

**SQLite Database** (tool execution history):
- macOS: `~/Library/Application Support/test_config_server/tool_logs.db`
- Linux: `~/.local/share/test_config_server/tool_logs.db`
- Windows: `%LOCALAPPDATA%\test_config_server\tool_logs.db`

## Common Issues

### ModuleNotFoundError

If you see `ModuleNotFoundError: No module named 'test_config_server'`:

1. Ensure you're in the project root directory
2. Check virtual environment is activated: `which python`
3. Reinstall: `uv sync`
4. Use full mcp path: `.venv/bin/mcp dev test_config_server/server/app.py`

### MCP Inspector Not Loading

1. Check server started without errors
2. Try a different port: `mcp dev test_config_server/server/app.py --port 5174`
...
1. Add your own tools in `test_config_server/tools/`
