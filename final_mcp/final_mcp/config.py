"""Configuration management for final mcp
...
class ServerConfig:
    ...
    oauth_backend_url: str = "http://localhost:8080"
    ...
    def __post_init__(self):
        ...
        if self.config_dir is None:
            self.config_dir = Path(platformdirs.user_config_dir("final_mcp"))
        if self.data_dir is None:
            self.data_dir = Path(platformdirs.user_data_dir("final_mcp"))
        if self.log_dir is None:
            self.log_dir = Path(platformdirs.user_log_dir("final_mcp"))
        ...
        self.log_file_path = self.log_dir / "final_mcp.log"
        self.database_path = self.data_dir / "final_mcp.db"
...
class UIConfig:
    ...
    # UI settings
    page_title: str = "final mcp Admin"
    page_icon: str = "🔧"
    layout: str = "wide"
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "UIConfig":
        ...
        return cls(
            page_title=data.get("page_title", "final mcp Admin"),
...
def get_platform_info() -> Dict[str, str]:
    ...
    return {
        "config_dir": str(platformdirs.user_config_dir("final_mcp")),
        "data_dir": str(platformdirs.user_data_dir("final_mcp")),
        "log_dir": str(platformdirs.user_log_dir("final_mcp")),
        "cache_dir": str(platformdirs.user_cache_dir("final_mcp")),
    }
