"""final mcp - MCP Server with SAAGA Decorators
...
from final_mcp.config import ServerConfig, get_config
from final_mcp.logging_config import setup_logging, logger
from final_mcp.log_system.correlation import (
...
from final_mcp.tools.example_tools import example_tools, parallel_example_tools
def create_mcp_server(config: Optional[ServerConfig] = None) -> FastMCP:
    ...
    from final_mcp.log_system.destinations import DestinationConfig
    ...
    import logging
    unified_logger = logging.getLogger('final_mcp')
    ...
    mcp_server = FastMCP(config.name or "final mcp")
...
def register_tools(mcp_server: FastMCP, config: ServerConfig) -> None:
    ...
    from final_mcp.decorators.exception_handler import exception_handler
    from final_mcp.decorators.tool_logger import tool_logger
    from final_mcp.decorators.type_converter import type_converter
    from final_mcp.decorators.parallelize import parallelize
    ...
    from final_mcp.tools.reddit_backend_tools import oauth_backend_tools
...
def main(port: int, transport: str) -> int:
    """Run the final mcp server with specified transport."""
