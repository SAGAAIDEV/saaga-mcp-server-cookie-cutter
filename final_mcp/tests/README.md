...
uv run pytest tests/ --cov=final_mcp --cov-report=term --cov-report=html
...
uv run pytest tests/ --cov=final_mcp --cov-report=html
...
import pytest
from final_mcp.tools.my_tools import my_new_tool
...
uv run pytest tests/ --cov=final_mcp --cov-fail-under=40

# Generate coverage report for CI artifacts
uv run pytest tests/ --cov=final_mcp --cov-report=xml
```
...
4. Check coverage hasn't decreased: `uv run pytest tests/ --cov=final_mcp`
