# MCP Inspector Testing Guide for final mcp
...
which mcp
# Should show: /path/to/final_mcp/.venv/bin/mcp
...
1. Use the full path to your venv's mcp:
   ```bash
   .venv/bin/mcp dev final_mcp/server/app.py
   ```
...
```bash
mcp dev final_mcp/server/app.py
```

### SSE Transport
```bash
mcp dev final_mcp/server/app.py --transport sse
```
...
```bash
mcp dev final_mcp/server/app.py --transport streamable-http
```
...
**Text Logs**:
- macOS: `~/Library/Logs/mcp-servers/final_mcp.log`
- Linux: `~/.local/state/mcp-servers/logs/final_mcp.log`
- Windows: `%LOCALAPPDATA%\mcp-servers\logs\final_mcp.log`

**SQLite Database** (tool execution history):
- macOS: `~/Library/Application Support/final_mcp/tool_logs.db`
- Linux: `~/.local/share/final_mcp/tool_logs.db`
- Windows: `%LOCALAPPDATA%\final_mcp\tool_logs.db`
...
```bash
streamlit run final_mcp/ui/app.py
```
...
If you see `ModuleNotFoundError: No module named 'final_mcp'`:

1. Ensure you're in the project root directory
2. Check virtual environment is activated: `which python`
3. Reinstall: `uv sync`
4. Use full mcp path: `.venv/bin/mcp dev final_mcp/server/app.py`

### MCP Inspector Not Loading

1. Check server started without errors
2. Try a different port: `mcp dev final_mcp/server/app.py --port 5174`
...
1. Add your own tools in `final_mcp/tools/`
