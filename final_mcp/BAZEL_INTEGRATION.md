...
bazel build //final_mcp:final_mcp
```
...
```bash
# Run the MCP server
bazel run //final_mcp:final_mcp

# Run with arguments
bazel run //final_mcp:final_mcp -- --help
```
...
- **py_binary**: `final_mcp` - The executable MCP server
- **py_library**: `final_mcp_lib` - The library containing all source files
