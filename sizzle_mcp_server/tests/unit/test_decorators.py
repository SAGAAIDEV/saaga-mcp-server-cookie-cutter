...
from sizzle_mcp_server.decorators.sqlite_logger import (
    SQLiteLoggerSink, 
    initialize_sqlite_logging,
    log_tool_execution
)
from sizzle_mcp_server.config import ServerConfig
...
class TestToolLogger:
    ...
    async def test_successful_logging(self, temp_db_config):
        ...
        with patch('sizzle_mcp_server.decorators.tool_logger.log_tool_execution') as mock_log:
            @tool_logger
            async def test_tool(param: str) -> str:
...
class TestSQLiteLogger:
    ...
    def test_database_initialization(self, temp_db_path):
        ...
        with patch('sizzle_mcp_server.decorators.sqlite_logger.SQLiteLoggerSink._get_database_path') as mock_path:
    ...
    def test_log_tool_execution_function(self, temp_db_path):
        ...
        with patch.object(sink, '_db_path', temp_db_path):
            ...
            with patch('sizzle_mcp_server.decorators.sqlite_logger.logger') as mock_logger:
...
class TestIntegrationPattern:
    ...
    async def test_example_server_pattern(self):
        ...
        with patch('sizzle_mcp_server.decorators.tool_logger.log_tool_execution'):
