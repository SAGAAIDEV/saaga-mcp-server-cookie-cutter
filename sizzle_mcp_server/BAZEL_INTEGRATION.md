...
bazel build //sizzle_mcp_server:sizzle_mcp_server
```
...
```bash
# Run the MCP server
bazel run //sizzle_mcp_server:sizzle_mcp_server

# Run with arguments
bazel run //sizzle_mcp_server:sizzle_mcp_server -- --help
```
...
- **py_binary**: `sizzle_mcp_server` - The executable MCP server
- **py_library**: `sizzle_mcp_server_lib` - The library containing all source files
...
3. Add the dependency to the `deps` list in `BUILD.bazel`:
   ...
   deps = [
       # ... existing deps ...
       "@mcp_sizzle_mcp_server//new_package",
   ]
   ```
