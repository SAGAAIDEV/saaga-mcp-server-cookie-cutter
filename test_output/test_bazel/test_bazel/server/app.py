"""Test Bazel - MCP Server with SAAGA Decorators
...
from test_bazel.config import ServerConfig, get_config
from test_bazel.logging_config import setup_logging, logger
from test_bazel.tools.example_tools import example_tools, parallel_example_tools
def create_mcp_server(config: Optional[ServerConfig] = None) -> FastMCP:
    ...
    mcp_server = FastMCP(config.name or "Test Bazel")
...
def register_tools(mcp_server: FastMCP, config: ServerConfig) -> None:
    ...
    from test_bazel.decorators.exception_handler import exception_handler
    from test_bazel.decorators.tool_logger import tool_logger
    from test_bazel.decorators.parallelize import parallelize
...
def main(port: int, transport: str) -> int:
    """Run the Test Bazel server with specified transport."""
    try:
        if transport == "stdio":
