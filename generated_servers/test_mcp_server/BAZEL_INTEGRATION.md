...
bazel build //test_mcp_server:test_mcp_server
```
...
```bash
# Run the MCP server
bazel run //test_mcp_server:test_mcp_server

# Run with arguments
bazel run //test_mcp_server:test_mcp_server -- --help
```
...
- **py_binary**: `test_mcp_server` - The executable MCP server
- **py_library**: `test_mcp_server_lib` - The library containing all source files
...
3. Add the dependency to the `deps` list in `BUILD.bazel`:
   ...
   deps = [
       # ... existing deps ...
       "@mcp_test_mcp_server//new_package",
   ]
   ```
