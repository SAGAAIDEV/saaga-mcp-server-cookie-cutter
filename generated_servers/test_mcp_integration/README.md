...
Simply tell your AI assistant: *"I have a Test MCP Integration project. Please read and follow [WORKING_WITH_SAAGA_PROMPT.md](WORKING_WITH_SAAGA_PROMPT.md) to help me understand and work with this MCP server."*
...
uv run mcp dev test_mcp_integration/server/app.py
```
...
uv run python -m test_mcp_integration.server.app

# Run with custom log level
uv run python -m test_mcp_integration.server.app --log-level DEBUG

# Run the server directly
uv run python test_mcp_integration/server/app.py

uv run test_mcp_integration-server
```
...
uv run python -m test_mcp_integration.server.app --transport sse --port 3001

# Run with custom host and port
uv run python -m test_mcp_integration.server.app --transport sse --host 0.0.0.0 --port 8080
```

### Command Line Options

```bash
uv run python -m test_mcp_integration.server.app --help
```
...
```json
{
  "mcpServers": {
    "test_mcp_integration": {
      "command": "uv",
      "args": ["run", "python", "-m", "test_mcp_integration.server.app"],
      "cwd": "/path/to/test_mcp_integration"
    }
  }
}
```
...
```json
{
  "mcpServers": {
    "test_mcp_integration": {
      "command": "uv",
      "args": [
        "run", "python", "-m", "test_mcp_integration.server.app",
        "--log-level", "DEBUG"
      ],
      "cwd": "/path/to/test_mcp_integration",
      "env": {
...
```json
{
  "mcpServers": {
    "test_mcp_integration": {
      "command": "/path/to/test_mcp_integration/.venv/bin/python",
      "args": ["-m", "test_mcp_integration.server.app"]
    }
  }
}
```
...
```json
{
  "mcpServers": {
    "test_mcp_integration": {
      "command": "uv",
      "args": ["--directory=/path/to/test_mcp_integration", "run" ,"test_mcp_integration-server"]
    }
  }
}
```
...
When working with this Test MCP Integration MCP server in an AI coding assistant (like Claude, Cursor, or GitHub Copilot):
...
5. **Logging**: Check logs at the platform-specific data directory for debugging:
   - macOS: `~/Library/Application Support/test_mcp_integration/logs.db`
   - Linux: `~/.local/share/test_mcp_integration/logs.db`
   - Windows: `%APPDATA%/test_mcp_integration/logs.db`

### Common Tasks

**Adding a new tool:**
```python
# In test_mcp_integration/tools/my_new_tool.py
...
- **macOS**: `~/Library/Application Support/test_mcp_integration/`
- **Linux**: `~/.local/share/test_mcp_integration/`
- **Windows**: `%APPDATA%/test_mcp_integration/`
...
1. Create a new Python file in `test_mcp_integration/tools/`
...
```python
# test_mcp_integration/tools/my_tool.py
...
```bash
# Format code
black test_mcp_integration/
isort test_mcp_integration/

# Lint code
flake8 test_mcp_integration/
mypy test_mcp_integration/
```
