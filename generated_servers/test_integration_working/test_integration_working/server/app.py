"""Test Integration Working - MCP Server with SAAGA Decorators
...
from test_integration_working.config import ServerConfig, get_config
from test_integration_working.logging_config import setup_logging, logger
from test_integration_working.tools.example_tools import example_tools, parallel_example_tools
def create_mcp_server(config: Optional[ServerConfig] = None) -> FastMCP:
    ...
    mcp_server = FastMCP(config.name or "Test Integration Working")
...
def register_tools(mcp_server: FastMCP, config: ServerConfig) -> None:
    ...
    from test_integration_working.decorators.exception_handler import exception_handler
    from test_integration_working.decorators.tool_logger import tool_logger
    from test_integration_working.decorators.parallelize import parallelize
...
def main(port: int, transport: str) -> int:
    """Run the Test Integration Working server with specified transport."""
    try:
        if transport == "stdio":
