...
from minimal_server.decorators.exception_handler import exception_handler
from minimal_server.decorators.tool_logger import tool_logger
from minimal_server.decorators.parallelize import parallelize
from minimal_server.decorators.sqlite_logger import (
    SQLiteLoggerSink, 
    initialize_sqlite_logging,
    log_tool_execution
)
from minimal_server.config import ServerConfig
...
class TestToolLogger:
    ...
    async def test_successful_logging(self, temp_db_config):
        ...
        with patch('minimal_server.decorators.sqlite_logger.log_tool_execution') as mock_log:
            @tool_logger
            async def test_tool(param: str) -> str:
...
class TestSQLiteLogger:
    ...
    def test_database_initialization(self, temp_db_path):
        ...
        with patch('minimal_server.decorators.sqlite_logger.SQLiteLoggerSink._get_database_path') as mock_path:
...
class TestIntegrationPattern:
    ...
    async def test_example_server_pattern(self):
        ...
        with patch('minimal_server.decorators.sqlite_logger.log_tool_execution'):
